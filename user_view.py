...
from auth import authenticate, get_user_details, add_user, delete_user, get_student_grades, get_student_eca, update_student_profile,update_eca,update_grades,read_grades_from_file
...
def login():
    ...
    role = authenticate(username, password,role)
    if role:
        ...
        messagebox.showerror("Login Failed", "Invalid username or password or role")
...
def admin_dashboard(user):
    ...
    def update_grades_ui():
        """
        UI for updating a student's grades.
        """
        def submit():
            username = username_entry.get()
            new_grades = grades_entry.get().split(",")  # Comma-separated values
            if update_grades(username, new_grades):
        ...
        grades_window.title("Update Grades")
        grades_window.geometry("350x200")

        tk.Label(grades_window, text="Username:").pack(pady=5)
        username_entry = tk.Entry(grades_window)
        username_entry.pack(pady=5)

        tk.Label(grades_window, text="Enter Grades (comma-separated):").pack(pady=5)
        grades_entry = tk.Entry(grades_window)
        grades_entry.pack(pady=5)

        tk.Button(grades_window, text="Submit", command=submit).pack(pady=10)

    def update_eca_ui():
        """
        UI for updating a student's extracurricular activities.
        """
        def submit():
            username = username_entry.get()
            new_eca = eca_entry.get().split(",")  # Comma-separated values
            if update_eca(username, new_eca):
                messagebox.showinfo("Success", "ECA updated successfully!")
                eca_window.destroy()
            else:
                messagebox.showerror("Error", "Failed to update ECA.")

        eca_window = tk.Toplevel()
        eca_window.title("Update ECA")
        eca_window.geometry("350x200")

        tk.Label(eca_window, text="Username:").pack(pady=5)
        username_entry = tk.Entry(eca_window)
        username_entry.pack(pady=5)

        tk.Label(eca_window, text="Enter Activities (comma-separated):").pack(pady=5)
        eca_entry = tk.Entry(eca_window)
        eca_entry.pack(pady=5)

        tk.Button(eca_window, text="Submit", command=submit).pack(pady=10)
    ...
    tk.Button(admin_window, text="Update Grades", command=update_grades_ui).pack(pady=10)
    tk.Button(admin_window, text="Update ECA", command=update_eca_ui).pack(pady=10)
    tk.Button(admin_window, text="Analytics", command=open_analytics).pack(pady=10)
...
def student_dashboard(user):
    ...
    def visualize_grades():
        grades = read_grades_from_file(user.username)
        if not grades:
            messagebox.showinfo("No Data", "No grades available to visualize.")
            return
        ...
        plt.ylim(0, 100)
        plt.title(f"{user.full_name}'s Grades")
        plt.xlabel("Subjects")
        plt.ylabel("Marks")
        plt.tight_layout()
        plt.show()
    ...
    tk.Button(student_window, text="Visualize Grades",
              command=visualize_grades).pack(pady=10)
...
def main():
    ...
    tk.Label(frame, text="Role:").pack(pady=5)
    global role_entry
    role_entry = tk.Entry(frame)
